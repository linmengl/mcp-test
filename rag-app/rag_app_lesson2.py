...
def generate_process_ds(prompt):
    try:
        # 构造 Ollama 请求
        payload = {
            "model": MODEL_NAME,
...
def generate_process_qwe(prompt):
    """
...
def main():
    ...
    # 生成模型的Prompt
    prompt = getContent(query, retrieval_chunks);
    
    # 生成流程：调用Qwen大模型生成响应
    # generate_process_qwe(prompt)
    
    # 生成流程：调用ds大模型生成响应
    result = generate_process_ds(prompt)
